module RodTheBot
  class EdgeMatchupWorker
    ...
    def perform(game_id)
      ...
      your_zone_data = Nhl::EdgeClient.fetch_team_zone_time_details(your_team_id, game_id: game_id)
      opp_zone_data = Nhl::EdgeClient.fetch_team_zone_time_details(opponent_team_id, game_id: game_id)
      ...
      retry_job(e, game_id: game_id, operation: "edge_matchup")
    end
    ...
    def format_matchup_post(your_data, opp_data, your_team_abbrev, opponent_team_abbrev, include_shot_diff: true)
      ...
      your_oz_pct = (your_all["offensiveZonePctg"] * 100).round(1)
      your_oz_rank = your_all["offensiveZoneRank"]
      your_dz_pct = (your_all["defensiveZonePctg"] * 100).round(1)
      your_dz_rank = your_all["defensiveZoneRank"]

      opp_oz_pct = (opp_all["offensiveZonePctg"] * 100).round(1)
      opp_oz_rank = opp_all["offensiveZoneRank"]
      opp_dz_pct = (opp_all["defensiveZonePctg"] * 100).round(1)
      opp_dz_rank = opp_all["defensiveZoneRank"]
