module RodTheBot
  module EdgeReplay
    class Renderer
      ...
      def initialize(source: Source.new, command_runner: CommandRunner.new)
        @source = source
        @command_runner = command_runner
      end

      def render(selected, options, frames_dir, tmpdir)
      ...
      attr_reader :source, :command_runner

      def run_cmd!(command, label)
        command_runner.run(command, label: label)
      end
    end
  end
end
